...
class Player:
	...
	async def get_input_from_prompt(self, prompt: str) -> str:
		await self.send_message_to_user(prompt)
	...
	async def getCardChoiceFromPlayer(self) -> Card:
		...
		while choice is None:
			await self.send_message_to_user(f'Please input a number between 0 and {self._hand.size - 1} to select an available card from your hand.')
			choice = await self._hand.getCard(self.get_input_from_prompt('What card do you want to play?\t'))
		return choice
	...
	async def getMoveChoiceFromPlayer(self, options : list[Move]) -> Move:
		...
		while choice not in [str(i) for i in range(len(options))]:
			await self.send_message_to_user('Please input a number between 0 and {len(options) - 1} to select an available move.')
			choice = await self.get_input_from_prompt('What move do you want to play?\t')
	...
	async def getSevenMoveFromPlayer(self, board : Board) -> None:
		...
		moves = []
		board.saveState()
		await self.send_message_to_user('Print select the moves you want to do in your seven-split:')
		...
		while counter < 7:
			...
			counter += 1
		await self.send_message_to_user(f'Selected moves for seven split : {moves}')
	...
	async def requestCardExchange(self) -> Card:
		await self.send_message_to_user(f'Player {self._name}, here are the cards in your hand: {"  ||  ".join([str(index) + " - " + str(card) for index,card in enumerate(self._hand.cards)])}')
